...
export default function CrmWorkspaceV2({session,initialLeads}:{session:CrmSession;initialLeads:Lead[]}){
 ...
 function invoice(l?:Lead){const p=new URLSearchParams();if(l){p.set("leadId",String(l.id||""));p.set("name",String(l.name||""));p.set("dest",String(l.dest||l.destination||""));p.set("contact",String(l.phone||""));p.set("email",String(l.email||""));p.set("salesperson",String(l.salesperson||session.profile.salesperson||""));p.set("pax",String(l.travellers??l.trav??""));p.set("start",String(l.travel_date||l.date||""))}router.push(`/crm/invoice${p.toString()?`?${p.toString()}`:""}`)}
 ...
 return <main className="crm-v2-shell">
  ...
  <section className="crm-v2-content"><header className="crm-v2-top"><div className="mobile-title"><span>ON A TRIP HOLIDAYS</span><b>{view==="dashboard"?"Sales Overview":view==="followups"?"Follow-ups":view==="bookings"?"Bookings":"Lead Management"}</b></div><div className="v2-top-search"><span>⌕</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search leads, phone, destination…"/></div><div className="crm-v2-top-actions"><button onClick={()=>itinerary()}>✨ AI Itinerary</button><button onClick={()=>invoice()}>▣ Invoice</button><button className="primary" onClick={newLead}>＋ New Lead</button></div></header>
   ...
   {view==="dashboard"&&<section className="crm-v2-page"><div className="v2-hero"><div><span>SALES COMMAND CENTER</span><h1>Know every lead. Close every opportunity.</h1><p>One workspace for enquiries, follow-ups, bookings and customer actions.</p></div><div className="v2-hero-actions"><button className="primary" onClick={newLead}>＋ Add lead</button><button onClick={()=>setView("leads")}>Open pipeline →</button></div></div><div className="v2-kpis"><button onClick={()=>setView("leads")}><span>Total pipeline</span><b>{active.length}</b><small>Active enquiries</small></button><button onClick={()=>setView("followups")}><span>Follow-ups</span><b>{followups.length}</b><small>{overdue.length} overdue</small></button><button onClick={()=>{setStage("Hot");setView("leads")}}><span>Hot opportunities</span><b>{hot.length}</b><small>Hot + urgent</small></button><button onClick={()=>setView("bookings")}><span>Bookings</span><b>{bookings.length}</b><small>{conversion}% conversion</small></button><button><span>Booked revenue</span><b>₹{Math.round(bookedValue).toLocaleString("en-IN")}</b><small>Recorded lead value</small></button></div><div className="v2-main-grid"><div className="v2-card"><div className="v2-card-head"><div><span>PIPELINE</span><h2>Sales stages</h2></div><b>₹{Math.round(pipelineValue).toLocaleString("en-IN")}</b></div><div className="v2-stage-grid">{stages.map(s=><button key={s} onClick={()=>{setStage(s);setView("leads")}}><span>{s}</span><strong>{counts[s]||0}</strong><i style={{width:`${active.length?Math.max(5,Math.round(((counts[s]||0)/active.length)*100)):5}%`}}/></button>)}</div></div><div className="v2-card"><div className="v2-card-head"><div><span>RECENT ACTIVITY</span><h2>Latest leads</h2></div><button onClick={()=>setView("leads")}>View all →</button></div><div className="v2-recent">{recent.length?recent.map(l=><button key={String(l.id)} onClick={()=>editLead(l)}><span className="avatar">{initials(String(l.name||"Lead"))}</span><span><b>{l.name||"Unnamed lead"}</b><small>{l.dest||l.destination||"Destination not set"} · {dateText(l.created_at)}</small></span><strong>₹{Math.round(num(l.value)).toLocaleString("en-IN")}</strong></button>):<div className="v2-empty">No leads yet. Add your first lead.</div>}</div></div></div></section>}
   {view!=="dashboard"&&<section className="crm-v2-page"><div className="v2-list-head"><div><span>{view==="leads"?"LEAD PIPELINE":view==="followups"?"FOLLOW-UP QUEUE":"BOOKING PIPELINE"}</span><h1>{view==="leads"?"All enquiries":view==="followups"?"Follow-ups":"Booked leads"}</h1></div><button className="primary" onClick={newLead}>＋ New Lead</button></div><div className="v2-filters"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search…"/><select value={stage} onChange={e=>setStage(e.target.value)}><option>All</option>{stages.map(s=><option key={s}>{s}</option>)}</select><select value={source} onChange={e=>setSource(e.target.value)}><option>All</option>{sources.map(s=><option key={s}>{s}</option>)}</select><select value={priority} onChange={e=>setPriority(e.target.value)}><option>All</option><option>Normal</option><option>High</option><option>Urgent</option></select>{isAdmin&&<select value={owner} onChange={e=>setOwner(e.target.value)}><option>All salespeople</option>{owners.map(s=><option key={s}>{s}</option>)}</select>}<button onClick={exportCsv}>Export CSV</button></div><div className="v2-table"><div className="v2-table-head"><span>LEAD</span><span>DESTINATION</span><span>STATUS</span><span>FOLLOW-UP</span><span>VALUE</span><span>ACTIONS</span></div>{filtered.filter(l=>view==="leads"||view==="followups"&&followups.some(x=>String(x.id)===String(l.id))||view==="bookings"&&String(l.status)==="Booked").map(l=><div className="v2-row" key={String(l.id)}><div className="lead-cell"><span className="avatar">{initials(String(l.name||"Lead"))}</span><span><b>{l.name||"Unnamed lead"}</b><small>{l.phone||l.email||"No contact"}</small></span></div><span>{l.dest||l.destination||"—"}</span><span><select value={String(l.status||"New")} onChange={e=>updateStatus(l.id||"",e.target.value)}>{stages.map(s=><option key={s}>{s}</option>)}</select></span><span>{dateText(l.follow_up||l.follow)}</span><strong>₹{Math.round(num(l.value)).toLocaleString("en-IN")}</strong><div className="row-actions"><button onClick={()=>call(l)} title="Call">☎</button><button onClick={()=>whatsapp(l)} title="WhatsApp">WA</button><button onClick={()=>itinerary(l)} title="AI Itinerary">✨</button><button onClick={()=>invoice(l)} title="Invoice">▣</button><button onClick={()=>editLead(l)} title="Edit">✎</button>{isAdmin&&<button onClick={()=>moveToBin(l)} title="Move to bin">⌫</button>}</div></div>)}{filtered.length===0&&<div className="v2-empty">No matching leads.</div>}</div></section>}
   {showForm&&<div className="crm-v2-modal-backdrop" onMouseDown={e=>{if(e.currentTarget===e.target)closeForm()}}><form className="crm-v2-modal" onSubmit={saveLead}><div className="modal-head"><div><span>{selected?"EDIT LEAD":"NEW LEAD"}</span><h2>{selected?"Update enquiry":"Create enquiry"}</h2></div><button type="button" onClick={closeForm}>×</button></div><div className="form-grid"><label>Name<input required value={form.name} onChange={e=>setField("name",e.target.value)}/></label><label>Phone<input value={form.phone} onChange={e=>setField("phone",e.target.value)}/></label><label>Email<input type="email" value={form.email} onChange={e=>setField("email",e.target.value)}/></label><label>Destination<input required value={form.destination} onChange={e=>setField("destination",e.target.value)}/></label><label>Status<select value={form.status} onChange={e=>setField("status",e.target.value)}>{stages.map(s=><option key={s}>{s}</option>)}</select></label><label>Source<select value={form.source} onChange={e=>setField("source",e.target.value)}>{sources.map(s=><option key={s}>{s}</option>)}</select></label><label>Branch<input value={form.branch} onChange={e=>setField("branch",e.target.value)}/></label><label>Priority<select value={form.priority} onChange={e=>setField("priority",e.target.value)}><option>Normal</option><option>High</option><option>Urgent</option></select></label><label>Lead value<input type="number" min="0" value={form.value} onChange={e=>setField("value",e.target.value)}/></label><label>Travel date<input type="date" value={form.travelDate} onChange={e=>setField("travelDate",e.target.value)}/></label><label>Travellers<input type="number" min="1" value={form.travellers} onChange={e=>setField("travellers",e.target.value)}/></label><label>Follow-up<input type="date" value={form.followUp} onChange={e=>setField("followUp",e.target.value)}/></label><label className="full">Notes<textarea rows={4} value={form.notes} onChange={e=>setField("notes",e.target.value)}/></label></div><div className="modal-actions"><button type="button" onClick={closeForm}>Cancel</button><button className="primary" disabled={saving}>{saving?"Saving…":selected?"Save changes":"Create lead"}</button></div></form></div>}
  </section>
 </main>;
}
