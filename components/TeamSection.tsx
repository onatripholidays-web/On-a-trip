...
type Member={name:string;role:string;image:string;email:string;phone:string;objectPosition?:string};
...
function EmployeeImage({member}:{member:Member}){return <Image src={member.image} alt={`${member.name}, ${member.role}`} fill sizes="(max-width: 700px) 50vw, (max-width: 1100px) 25vw, 220px" loading="lazy" quality={68} style={member.objectPosition?{objectPosition:member.objectPosition}:undefined}/>}
export default function TeamSection(){const [openMember,setOpenMember]=useState<string|null>(null);return <section className={styles.teamSection} aria-labelledby="team-heading"><div className={styles.container}><h2 id="team-heading" className={styles.srOnly}>On A Trip Holidays Team</h2><div className={styles.teamGrid}>{members.map(member=>{const isOpen=openMember===member.name;const hasDetails=Boolean(member.email||member.phone);return <article className={`${styles.card} ${isOpen?styles.open:""}`} key={member.name}><div className={styles.photoFrame}><EmployeeImage member={member}/></div><div className={styles.cardInfo}><div className={styles.name}>{member.name}</div><div className={styles.roleRow}><strong>{member.role}</strong>{hasDetails?<button type="button" className={styles.expandButton} aria-label={`${isOpen?"Hide":"Show"} official details for ${member.name}`} aria-expanded={isOpen} onClick={()=>setOpenMember(isOpen?null:member.name)}><span aria-hidden="true">⌄</span></button>:null}</div></div><div className={`${styles.detailsWrap} ${isOpen?styles.detailsOpen:""}`}><div className={styles.details}>{member.email?<a href={`mailto:${member.email}`}>✉ <span>{member.email}</span></a>:null}{member.phone?<a href={`tel:${member.phone}`}>☎ <span>{member.phone}</span></a>:null}</div></div></article>})}</div></div></section>}
