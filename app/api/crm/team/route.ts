...
export async function POST(req:Request){
 ...
 if(!service)return NextResponse.json({error:"Server key not configured"},{status:503});
 ...
 let data:any=null;try{data=await r.json()}catch{data=null}if(!r.ok)return NextResponse.json({error:data?.message||data?.details||"Could not add salesperson"},{status:r.status});
