...
async function activity(ctx:any,type:string,subject:string,body:string,enquiryId:string){
 const authKey=ctx.service||ctx.access;
 try{await fetch(`${ctx.url}/rest/v1/crm_activities`,{method:"POST",headers:{apikey:authKey,Authorization:`Bearer ${authKey}`,"Content-Type":"application/json",Prefer:"return=minimal"},body:JSON.stringify({enquiry_id:enquiryId,actor_id:ctx.session.user.id,type,subject,body,metadata:{}})})}catch{}
}
...
export async function PATCH(req:Request){
 ...
 const beforeRes=await fetch(`${ctx.url}/rest/v1/enquiries?select=id,name,status,salesperson&id=eq.${encodeURIComponent(id)}${ownerFilter}`,{headers:{apikey:rpcAuth,Authorization:`Bearer ${rpcAuth}`},cache:"no-store"});
 ...
 const r=await fetch(`${ctx.url}/rest/v1/enquiries?id=eq.${encodeURIComponent(id)}${ownerFilter}`,{method:"PATCH",headers:{apikey:rpcAuth,Authorization:`Bearer ${rpcAuth}`,"Content-Type":"application/json",Prefer:"return=representation"},body:JSON.stringify(patch)});
 const text=await r.text();let data:any=null;try{data=text?JSON.parse(text):null}catch{data=text}
...
export async function DELETE(req:Request){
 ...
 const rpcAuth=ctx.service||ctx.access;
 const r=await fetch(`${ctx.url}/rest/v1/enquiries?id=eq.${encodeURIComponent(body.id)}`,{method:"PATCH",headers:{apikey:rpcAuth,Authorization:`Bearer ${rpcAuth}`,"Content-Type":"application/json"},body:JSON.stringify({status:"Deleted"})});
