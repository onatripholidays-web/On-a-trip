...
export async function POST(req:Request){
 ...
 const user=await rest(url,key,"crm_sales_team?auth_user_id=eq."+encodeURIComponent(userId)+"&select=auth_user_id,name,email,is_active&limit=1");
 const rows=await user.json().catch(()=>[]);
 const person=Array.isArray(rows)?rows[0]:null;
 if(!person||person.is_active===false)return NextResponse.json({error:"Salesperson not found or inactive"},{status:404});
 const lead=await rest(url,key,"enquiries?id=eq."+encodeURIComponent(id),{method:"PATCH",headers:{Prefer:"return=representation"},body:JSON.stringify({assigned_to:userId,salesperson:person.name})});
