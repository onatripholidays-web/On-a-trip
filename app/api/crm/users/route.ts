...
export async function POST(req:Request){
 ...
 const body=await req.json().catch(()=>({}));const name=String(body.name||"").trim();const email=String(body.email||"").trim().toLowerCase();const password=String(body.password||"");const role=String(body.role||"salesperson").trim();const permissions=body.permissions&&typeof body.permissions==="object"?body.permissions:{};
 const allowedRoles=["admin","salesperson","accountant","operations","manager","staff"]; if(name.length<2||!email||password.length<6||!allowedRoles.includes(role))return NextResponse.json({error:"Name, valid email and a password of at least 6 characters are required."},{status:400});
...
export async function PUT(req:Request){
 ...
 const {url}=crmSupabaseConfig();const service=process.env.SUPABASE_SECRET_KEY||process.env.SUPABASE_SERVICE_ROLE_KEY;if(!service)return NextResponse.json({error:"Server user-management key is not configured."},{status:503});
 const body=await req.json().catch(()=>({}));const id=String(body.id||"").trim();const name=String(body.name||"").trim();const email=String(body.email||"").trim().toLowerCase();const role=String(body.role||"").trim();const permissions=body.permissions&&typeof body.permissions==="object"?body.permissions:{};
 ...
 const profileRole=role==="admin"?"admin":role==="salesperson"?"sales":role==="accountant"?"accounts":role==="operations"?"operations":role==="manager"?"manager":"staff";
 ...
 const crm=await rest(url,service,`crm_users?user_id=eq.${encodeURIComponent(id)}`,{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify({email,role,salesperson:role==="salesperson"?name:null,permissions})});
