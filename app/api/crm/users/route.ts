...
export async function POST(req:Request){
 ...
 if(service.split(".").length===3) authHeaders.Authorization=`Bearer ${service}`;
 ...
 if(!profile.ok){await fetch(`${url}/auth/v1/admin/users/${encodeURIComponent(userId)}`,{method:"DELETE",headers:{apikey:service}});return NextResponse.json({error:"Could not create the CRM profile."},{status:500});}
