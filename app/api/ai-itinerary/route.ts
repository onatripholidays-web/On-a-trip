...
const inputSchema=z.object({
  guest:z.string().trim().max(120).optional().default("Guest"),
  dest:z.string().trim().min(2).max(120),
  days:z.union([z.string(),z.number()]).transform(Number).pipe(z.number().int().min(1).max(30)),
  pax:z.union([z.string(),z.number()]).transform(Number).pipe(z.number().int().min(1).max(100)),
  start:z.string().trim().max(120).optional().default("TBC"),
  dates:z.string().trim().max(120).optional().default("TBC"),
  language:z.string().trim().max(40).optional().default("English"),
  type:z.string().trim().max(40).optional().default("Custom"),
  hotel:z.string().trim().max(120).optional().default("3 Star"),
  vehicle:z.string().trim().max(120).optional().default("Private Car"),
  meals:z.string().trim().max(120).optional().default("Breakfast & Dinner"),
  budget:z.string().trim().max(200).optional().default("Price to be quoted"),
  special:z.string().trim().max(1000).optional().default("None"),
  instruction:z.string().trim().max(1000).optional().default("None"),
});

async function authorized(){return Boolean(await getCrmSession())}
...
export async function POST(req:Request){
  ...
  try{
    ...
    if(!parsed.success)return NextResponse.json({error:"Please check the itinerary details."},{status:400});
    const r=parsed.data;
    const destination=r.dest;
    const days=r.days;
    const pax=r.pax;
