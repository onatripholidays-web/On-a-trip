-- On A Trip CRM Travel OS enhancement foundation
-- Adds the operational/financial/marketing primitives needed for a full travel-agency CRM.
...
create table if not exists public.crm_trip_travellers (
  id uuid primary key default gen_random_uuid(),
  trip_id uuid not null references public.crm_trips(id) on delete cascade,
  full_name text not null,
  traveller_type text not null default 'adult' check (traveller_type in ('adult','child','infant')),
  phone text,
  email text,
  dob date,
  gender text,
...
create table if not exists public.crm_trip_vouchers (
  id uuid primary key default gen_random_uuid(),
  trip_id uuid not null references public.crm_trips(id) on delete cascade,
  service_id uuid references public.crm_trip_services(id) on delete set null,
...
create table if not exists public.crm_supplier_ledger (
  id uuid primary key default gen_random_uuid(),
  trip_id uuid references public.crm_trips(id) on delete set null,
  supplier_id uuid,
  reference_type text not null default 'supplier_bill',
  reference_no text,
...
create table if not exists public.crm_customer_documents (
  id uuid primary key default gen_random_uuid(),
  customer_id uuid,
  trip_id uuid references public.crm_trips(id) on delete cascade,
  traveller_id uuid references public.crm_trip_travellers(id) on delete cascade,
...
create table if not exists public.crm_sales_targets (
  ...
  bookings_achieved integer not null default 0,
  created_at timestamptz not null default now(),
...
create table if not exists public.crm_incentive_payouts (
  ...
  period_start date not null,
  period_end date not null,
  achievement_amount numeric(14,2) not null default 0,
...
select
  ...
  t.cost_amount,
  t.collected_amount,
  greatest(t.selling_amount - t.collected_amount, 0) as outstanding_amount,
  t.selling_amount - t.cost_amount as gross_profit,
  case when t.selling_amount > 0
    then round(((t.selling_amount - t.cost_amount) / t.selling_amount) * 100, 2)
    else 0 end as gross_margin_percent
from public.crm_trips t;

comment on table public.crm_trips is 'Trip-level operating record connecting enquiry, quotation, booking, services, payments and profitability.';
comment on table public.crm_lead_scores is 'Calculated lead temperature and scoring factors.';
comment on table public.crm_automation_rules is 'CRM trigger/condition/action automation definitions.';
comment on table public.crm_customer_portal_tokens is 'Revocable, expiring customer trip portal access tokens.';
